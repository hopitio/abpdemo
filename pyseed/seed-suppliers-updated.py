#!/usr/bin/env python3
"""
Updated Python script to seed 20 suppliers into the ABP Angular database.
This version matches the actual database table structure.
Requirements: pip install mysql-connector-python

Usage:
    python seed-suppliers-updated.py          # Check existing and skip if found
    python seed-suppliers-updated.py --force  # Force seed even if suppliers exist
    python seed-suppliers-updated.py --clear  # Clear existing data first
"""
...
def generate_uuid():
    """Generate a UUID with dashes for ABP compatibility"""
...
def insert_supplier(cursor, supplier_data):
    ...
    # Prepare the insert query based on actual table structure
    insert_query = """
...
def seed_suppliers(force=False, clear_existing=False):
    ...
    try:
        ...
        if existing_count > 0 and not force and not clear_existing:
